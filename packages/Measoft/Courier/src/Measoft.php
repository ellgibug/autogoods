...
class Measoft
{
    ...
    public function __construct($login = null, $password = null, $extra = null)
    {
        if ($login && $password && $extra) {
            ...
            die($login.', '.$password.', '.$extra.'Проверьте аутентификационные данные');
        }
    }
    ...
    public function createXML($order, $items)
    {
        ...
        $level++;
        $result .= $this->makeXMLNode('company', (isset($order['company']) ? $order['company'] : $order['person']), $level);
        ...
        if (isset($items) && $items) {
            ...
            foreach ($items as $item) {
                $result .= $this->makeXMLNode('item', $item['name'], $level, 'quantity="'.$item['quantity'].'" mass="'.$item['mass'].'" retprice="'.$item['retprice'].'"');
            }
            $level--;
    ...
    public function createXML4Regions($stname)
    {
        $level = 0;
        $result = $this->startXML();
        $result .= $this->makeXMLNode('regionlist', '', $level, '', 1);
            $level++;
            $result .= $this->makeXMLNode('conditions', '', $level, '', 1);
                $level++;
                    $result .= $this->makeXMLNode('namestarts', $stname, $level);
                $level--;
            $result .= $this->makeXMLNode('conditions', '', $level, '', 2);
            $level--;
        $result .= $this->makeXMLNode('regionlist', '', $level, '', 2);

        return $result;
    }

    public function createXML4Cities($stname)
    {
        $level = 0;
        $result = $this->startXML();
        $result .= $this->makeXMLNode('townlist', '', $level, '', 1);
            $level++;
            $result .= $this->makeXMLNode('conditions', '', $level, '', 1);
                $level++;
                    $result .= $this->makeXMLNode('namestarts', $stname, $level);
            ...
            $level--;
            $level++;
                $result .= $this->makeXMLNode('limit', '', $level, '', 1);
                $level++;
                    $result .= $this->makeXMLNode('limitcount', 20, $level);
                $level--;
                $result .= $this->makeXMLNode('limit', '', $level, '', 2);
            $level--;
        $result .= $this->makeXMLNode('townlist', '', $level, '', 2);

        return $result;
    }

    public function createXML4Streets($stname, $town)
    {
        $level = 0;
        $result = $this->startXML();
        $result .= $this->makeXMLNode('streetlist', '', $level, '', 1);
            $level++;
            $result .= $this->makeXMLNode('conditions', '', $level, '', 1);
            $level++;
                $result .= $this->makeXMLNode('town', $town, $level);
                $result .= $this->makeXMLNode('namestarts', $stname, $level);
                $level--;
                    $result .= $this->makeXMLNode('conditions', '', $level, '', 2);
                $level--;
            ...
            $result .= $this->makeXMLNode('limit', '', $level, '', 2);
            $level--;
        $result .= $this->makeXMLNode('streetlist', '', $level, '', 2);

        return $result;
    }

    public function createXML4PVZ($stname)
    {
        $level = 0;
        $result = $this->startXML();
        $result .= $this->makeXMLNode('pvzlist', '', $level, '', 1);
        $level++;
        $result .= $this->makeXMLNode('auth', '', $level, 'extra="'.$this->extra.'" login="'.$this->login.'" pass="'.$this->password.'"');
        $result .= $this->makeXMLNode('town', $stname, $level);
        $level--;
        $result .= $this->makeXMLNode('pvzlist', '', $level, '', 2);
    ...
    public function orderRequest4Regions($stname)
    {
        if (!$stname) {
            $this->errors = 'Пустой массив';
            return false;
        }
        $response = simplexml_load_string($this->sendRequest($this->createXML4Regions($stname)));
        return $response;
    }



    public function orderRequest4Cities($stname)
    {
        if (!$stname) {
            $this->errors = 'Пустой массив';
            return false;
        }
        $response = simplexml_load_string($this->sendRequest($this->createXML4Cities($stname)));
        return $response;
    }

    public function orderRequest4Streets($stname, $town)
    {
        if (!$stname || !$town) {
            $this->errors = 'Пустой массив';
            return false;
        }
        $response = simplexml_load_string($this->sendRequest($this->createXML4Streets($stname, $town)));
        return $response;
    }

    public function orderRequest4PVZ($stname)
    {
        if (!$stname) {
            $this->errors = 'Пустой массив';
            return false;
        }
        $response = simplexml_load_string($this->sendRequest($this->createXML4PVZ($stname)));
        return $response;
    }
